...
/*
 * Jenkinsfile for ansible-eos-mlag role
 *
 * Run the Ansible-Role-Test job against a commit or
 * pull request in the ansible-eos-mlag repo
 */
...
pipeline {
    ...
    environment {
        projectName = 'ansible-eos-mlag'
        emailTo = 'grybak@arista.com'
        emailFrom = 'grybak+jenkins@arista.com'
    }

    stages {
        stage ('Run tests for ansible-eos-mlag role') {
            when {
            ...
            steps {
                ...
                build job: 'gar-test-starter',
                      parameters: [
                          string(name: 'ROLE_NAME', value: 'ansible-eos-mlag'),
